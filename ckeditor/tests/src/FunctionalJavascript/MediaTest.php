...
class MediaTest extends WebDriverTestBase {
  ...
  public function testErrorMessages() {
    ...
    // Test when using the claro theme, an additional class is added in
    // claro/templates/content/media-embed-error.html.twig.
    $this->assertTrue($this->container->get('theme_installer')->install(['claro']));
    ...
    // Drupal 10 core themes do not support CKE4 anymore.
    if (version_compare(\Drupal::VERSION, '10.0.0-dev', '<')) {
      $assert_session->responseContains('css/classy/components/media-embed-error.css');
    }
  ...
  public function testPreviewUsesDefaultThemeAndIsClientCacheable() {
    ...
    $this->assertTrue($this->container->get('theme_installer')->install(['olivero']));
    $this->config('system.theme')
      ->set('default', 'olivero')
      ->set('admin', 'claro')
      ->save();
    ...
    $this->assertSame('olivero', $element->getAttribute('data-media-embed-test-active-theme'));
  ...
  public function testEmbedPreviewAccess($media_embed_enabled, $can_use_format) {
    ...
    if ($can_use_format) {
      ...
      if ($media_embed_enabled) {
        // The preview rendering, which in this test will use Claro's
