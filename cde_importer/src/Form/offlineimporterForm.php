...
class offlineimporterForm extends FormBase {
	
  public function getFormId() {
    return 'offline_importer_form';
  }
	...
	*/
		  ...
		  // Extract the relevant information from the product entity.
			...
			foreach($fields_mapped as $fields_mapped_key => $fields_mapped_val){	
				...
				if(!empty($_type) && $_type=="date"){
					...
					if(array_key_exists('0',$price_value)){
						$_product[$_field] = $price_value[0]['value'];
					}else{
						$_product[$_field]='';
					}
					
				}elseif(!empty($_type) && $_type=="price"){
			...
			if ($_product) {
				...
				foreach($fields_mapped as $fields_mapped_key => $fields_mapped_val){	
					...
					if(array_key_exists($fields_mapped_key,$row)){
						...
						$_product->get('field_offline_item')->setValue(1);
	...
	function loadProductAttributes() {
		...
		return $fields_mapped=[	
			...
			'ELIGIBLE CUSTOMERS' => ['field'=>'field_eligible_customers', 'type' => "users", 'taxonomy'=>''],
			'Supplier' => ['field'=>'field_supplier_reference', 'type' => "", 'taxonomy'=>''],
			'Supplier SKU' => ['field'=>'field_supplier_2_sku', 'type' => "", 'taxonomy'=>''],
			'Supplier Cost' => ['field'=>'field_supplier_cost', 'type' => "price", 'taxonomy'=>''],
			'Expiration Date' => ['field'=>'field_expiration_date', 'type' => "date", 'taxonomy'=>''],
		]; 
	}
