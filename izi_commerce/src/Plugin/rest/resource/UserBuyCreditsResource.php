 ...
 *
 * @RestResource(
 *   id = "user_buy_credits_resource",
 *   label = @Translation("User buy credits resource"),
 *   uri_paths = {
 *     "create" = "user/buy-credits",
 *   },
 * )
 */
class UserBuyCreditsResource extends ResourceBase {
  ...
  public function post(Request $request) {
	...
	if(!array_key_exists('creditsToBuy',$request_body) || empty($request_body['creditsToBuy']) ){
		$error_result = [
			'error' => "creditsToBuy is required"
		];
		return new ResourceResponse($error_result, 403); 
	}
	
	
	// Load the user entity based on the email address.
	$user = user_load_by_mail($request_body['mail']);
	...
	if ($user) {
		...
		$credits = $this->commerceCredits->updateCreditsApi($request_body['creditsToBuy'], $user_id, $request_body['mail']);
		$error_message = ['error' => ''];
		if (empty($credits)) {
