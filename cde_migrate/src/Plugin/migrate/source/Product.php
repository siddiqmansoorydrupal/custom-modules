 ...
 * )
 */
class Product extends DrupalSqlBase {
  ...
  public function prepareRow(Row $row) {
	...
	if(empty($title)){		
		...
		$row->setSourceProperty('title', $this->getResults("field_data_".$field, $product_id, $field.'_value'));
	}
	
	
	$fields = ['field_base_diameter', 'field_blue_devil_item_no', 'field_blue_devil_price', 'field_description', 'field_description_long_text', 'field_domestic_import', 'field_global_sort', 'field_grip_range' , 'field_head_height', 'field_in_stock', 'field_internal_notes', 'field_item_description', 'field_live_item', 'field_long_description', 'field_offline_item', 'field_pieces_per_lb', 'field_point', 'field_price_break_1', 'field_price_break_3', 'field_product_code', 'field_product_field', 'field_product_sku', 'field_qty_break_1', 'field_quantity_per_box', 'field_solr_text', 'field_sort_size', 'field_offline_item_status', 'field_supplier_2', 'field_supplier_2_cost', 'field_tab_base', 'field_tele_part', 'field_thickness', 'field_field_title_alias_product', 'field_title_alias', 'field_unit_of_measure', 'field_weight', 'field_weight_per_um', 'field_product_category', 'field_product_size', 'field_product_subcategory','field_supplier_sku'];
	
	foreach($fields as $field):
		$row->setSourceProperty($field, $this->getResults("field_data_".$field, $product_id, $field.'_value'));
	endforeach;
	
	$fields_multiple = ['field_part_cross_reference'];
	
	foreach($fields_multiple as $field):
		$row->setSourceProperty($field, $this->getResults("field_data_".$field, $product_id, $field.'_value',"array"));
	endforeach;
	...
	$fields_amount = ['field_5price', 'commerce_price', 'field_supplier_cost'];
	
	foreach($fields_amount as $field):
	
		$_price=$this->getResults("field_data_".$field, $product_id, $field.'_amount');
		if(empty($_price)){
			$_price=0;
		}
		if($_price != 0){
		...
		$row->setSourceProperty($field.'_amount', $_price);
		$row->setSourceProperty($field.'_currency_code', $this->getResults("field_data_".$field, $product_id, $field.'_currency_code'));
	endforeach;
	
	$fields_users = ['field_eligible_customers'];
	
	foreach($fields_users as $field):
		//$row->setSourceProperty($field, $this->getUsersResults("field_data_".$field, $product_id, $field.'_target_id'));
		$row->setSourceProperty("field_data_".$field, $product_id, $field.'_target_id',"array");
		
	endforeach;
	
	
	$fields_taxonomy = ['field_abnormal_head_diameter','field_body_material','field_category_taxonomy','field_cross_over', 'field_cross_over_2','field_cross_over_sub_sub_cat','field_diameter', 'field_distance_across_flats', 'field_drive', 'field_finish', 'field_grip_range_taxonomy', 'field_head_style', 'field_hex_washer', 'field_inner_diameter', 'field_internal_expander', 'field_item_description', 'field_length', 'field_live_item', 'field_long_description', 'field_material', 'field_metric_standard', 'field_metric_standard_', 'field_military_specification', 'field_name_overide', 'field_national_aerospace_standar', 'field_offline_item', 'field_outer_diameter', 'field_pieces_per_lb', 'field_pin_material', 'field_point', 'field_point_type', 'field_price_break_1', 'field_price_break_3', 'field_product_category_taxonomy', 'field_product_code', 'field_product_field', 'field_product_size_taxonomy', 'field_product_sku', 'field_qty_break_1', 'field_quantity_per_box', 'field_screw_material', 'field_shaft_diameter', 'field_shank', 'field_solr_text', 'field_sort_size', 'field_offline_item_status', 'field_style', 'field_sub_sub_cat', 'field_supplier_2', 'field_supplier_2_cost', 'field_tab_base', 'field_tek_point', 'field_tele_part', 'field_thickness', 'field_thickness_taxonomy', 'field_thread_length', 'field_thread_pitch', 'field_thread_type', 'field_field_title_alias_product', 'field_title_alias', 'field_type', 'field_unit_of_measure', 'field_washer_material', 'field_washer_outer_diameter', 'field_washer_type', 'field_weight', 'field_weight_per_um', 'field_weld_style', 'field_wings', 'field_product_category', 'field_product_size', 'field_product_subcategory', 'field_external_sleeve'];
	
	foreach($fields_taxonomy as $field):
		if(!in_array($field,$fields)){
			$row->setSourceProperty($field, $this->getTaxonomyResults("field_data_".$field, $product_id, $field.'_tid'));
		}	
		
	endforeach;
	
	$fields_content = ['field_supplier_reference'];
	
	foreach($fields_content as $field):
		if(!in_array($field,$fields)){
			$row->setSourceProperty($field, $this->getEntityResults("field_data_".$field, $product_id, $field.'_target_id'));
		}	
	...
	$fields = ['field_image_commerce_product'];
	
	foreach($fields as $field):
		$row->setSourceProperty($field, $this->getFileResults("field_data_".$field, $product_id, $field.'_fid'));
	endforeach;
	
	
	$fields = ['field_sort_sizes'=>'field_sort_size'];
	
	foreach($fields as $field_key=>$field):
		$row->setSourceProperty($field_key, $this->getResults("field_data_".$field, $product_id, $field.'_value'));
	endforeach;
	
	
	$fields = ['feeds_item', 'field_data_field_national_aero_space', 'field_product_category_term'];
