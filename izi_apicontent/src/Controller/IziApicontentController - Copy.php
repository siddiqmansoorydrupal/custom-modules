...
class IziApicontentController extends ControllerBase {
  ...
  private function izi_apicontent_build_audioplayer(array $audio_items, MtgObjectInterface $parent, array $context) {
    ...
    if (!empty($audio_items)) {
      ...
      if (isset($context['purchase']) && $context['purchase']) {
		...
		$variation_id = $this->getVariationIdBySku($sku);
	...
	function getVariationIdBySku($sku) {
	  ...
	  if (!empty($variation_ids)) {
		// Return the first variation ID found.
		return reset($variation_ids);
	  }
  ...
  protected function izi_apicontent_html_files_expired($object, $type = 'parent') {
    ...
    if ($type === 'parent') {
      ...
      $object_uri_folder = self::PARENT_CACHE_DESTINATION;
	  
	  if (\Drupal::currentUser()->isAuthenticated()) {
			$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
			$currentUserId = $currentUser->id();
			$object_uri_folder = self::PARENT_CACHE_DESTINATION . $currentUser->id()."/";
      ...
      // Including trailing slash.
		...
		if (\Drupal::currentUser()->isAuthenticated()) {
			$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
			$currentUserId = $currentUser->id();
			$object_uri_folder = self::CHILD_CACHE_DESTINATION . $currentUser->id()."/";
			
		}else{
			$object_uri_folder = self::CHILD_CACHE_DESTINATION;
		}
  ...
  private function izi_apicontent_save_parent_uuid_date($parent) {
    ...
    try {
		...
		if (\Drupal::currentUser()->isAuthenticated()) {
			$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
			$currentUserId = $currentUser->id();
			$destination = self::PARENT_CACHE_DESTINATION . $currentUser->id()."/".$parent_uuid . '_' . $language;
			
		}else{
			$destination = self::PARENT_CACHE_DESTINATION . $parent_uuid . '_' . $language;
		}
    ...
    if (!empty($exhibit)) {
      ...
      try {
		...
		if (\Drupal::currentUser()->isAuthenticated()) {
			$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
			$currentUserId = $currentUser->id();
			$destination = self::CHILD_CACHE_DESTINATION . $currentUser->id()."/";
			
		}else{
			$destination = self::CHILD_CACHE_DESTINATION;
		}
    ...
    if (!empty($child)) {
      ...
      try {
		...
		if (\Drupal::currentUser()->isAuthenticated()) {
			$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
			$currentUserId = $currentUser->id();
			$destination = self::CHILD_CACHE_DESTINATION . $currentUser->id()."/".$child['#uuid'] . '_' . $child['#language'] . '.html';
			
		}else{
			$destination = self::CHILD_CACHE_DESTINATION . $child['#uuid'] . '_' . $child['#language'] . '.html';
		}
  ...
  protected function checkDirectories() {
    if (!is_dir(self::CHILD_CACHE_DESTINATION)) {
      $destination = self::CHILD_CACHE_DESTINATION;
	...
	if (\Drupal::currentUser()->isAuthenticated()) {
		...
		$currentUser = \Drupal\user\Entity\User::load(\Drupal::currentUser()->id());
		$currentUserId = $currentUser->id();
		$destination .= $currentUserId.'/';
		$this->fileSystem->prepareDirectory($destination, FileSystemInterface::CREATE_DIRECTORY);
	}
	
	
    if (!is_dir(self::PARENT_CACHE_DESTINATION)) {
      $destination = self::PARENT_CACHE_DESTINATION;
      $this->fileSystem->prepareDirectory($destination, FileSystemInterface::CREATE_DIRECTORY);
    }
